...
@Component({
  ...
  template: `
    <nav class="navbar">
      ...
      <div class="navbar-links">
        ...
        <a [routerLink]="['/polizas', anioActual, 1]"  routerLinkActive="active">Pólizas</a>
        <a [routerLink]="['/cedis',   anioActual, 10]" routerLinkActive="active">CEDIS</a>
        @if (auth.esAdmin) {
          <a routerLink="/admin" routerLinkActive="active" class="link-admin">⚙ Admin</a>
        }
      </div>
      ...
      <div class="navbar-user">
        <div class="user-info">
          ...
          <span class="user-rol" [class]="auth.esAdmin ? 'chip-admin' : 'chip-tec'">
  ...
  styles: [`
    ...
    .chip-admin   { font-size: 10px; padding: 1px 8px; border-radius: 20px;
                    background: rgba(253,224,71,.15); color: #FDE047;
                    border: 1px solid rgba(253,224,71,.3); }
    .chip-tec     { font-size: 10px; padding: 1px 8px; border-radius: 20px;
